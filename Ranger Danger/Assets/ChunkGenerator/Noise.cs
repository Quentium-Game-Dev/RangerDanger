...
public static class Noise
{
	...
	private const int hashMask = 255; // = hash length - 1
	...
	public static void RandomiseHash()
    {
		...
		int hashLength = 2 * (hashMask + 1);
		hash = new int[hashLength];
		for (int i = 0; i < hashLength/2; i++)
        {
			hash[i] = numbers[i];
			hash[i + hashLength/2] = numbers[i];
		}
	}
	...
	public static NoiseSample Value2D(Vector3 point, float frequency)
	{
		...
		// = h00 + (h10 - h00)tx + (h01 + (h11 - h01)tx - (h00 + (h10 - h00)tx))ty
		// = Mathf.Lerp(Mathf.Lerp(h00, h10, tx), Mathf.Lerp(h01, h11, tx), ty);
	...
	public static NoiseSample Simplex1D(Vector3 point, float frequency)
	{
		...
		sample.derivative *= frequency;
		return sample * simplexScale1D;
	}
	private static float simplexScale1D = 64f / 27f;
